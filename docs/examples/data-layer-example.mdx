---
sidebar_position: 6
---

import {
  DataLayerGeoJsonExample,
  DataLayerGeoJsonUrlExample,
} from "@site/src/components/examples/dataLayerGeoJsonExample";
import { DataLayerGpxExample } from "@site/src/components/examples/dataLayerGpxExample";
import BrowserOnly from "@docusaurus/BrowserOnly";

# 데이터 레이어 예제

GeoJSON과 GPX 데이터를 로딩해서 지도 위에 표시하는 예제입니다.

## GeoJSON 라이브 예제 (URL)

<BrowserOnly>{() => <DataLayerGeoJsonUrlExample />}</BrowserOnly>

## GeoJSON 코드 (URL)

```tsx
import { useState } from "react";
import useBaseUrl from "@docusaurus/useBaseUrl";
import { DataLayer, Map } from "@rousen/react-naver-maps";

function DataLayerExample() {
  const [useAltStyle, setUseAltStyle] = useState(false);
  const geojsonUrl = useBaseUrl("/data/sample-geojson.json");
  const style = useAltStyle
    ? {
        fillColor: "#f97316",
        fillOpacity: 0.25,
        strokeColor: "#ea580c",
        strokeOpacity: 0.9,
        strokeWeight: 3,
      }
    : {
        fillColor: "#60a5fa",
        fillOpacity: 0.35,
        strokeColor: "#2563eb",
        strokeOpacity: 0.9,
        strokeWeight: 2,
      };

  return (
    <div>
      <button type="button" onClick={() => setUseAltStyle((prev) => !prev)}>
        GeoJSON 스타일 변경
      </button>
      <div style={{ width: "100%", height: "400px", marginTop: "10px" }}>
        <Map
          ncpKeyId="your-ncp-key-id"
          initialOptions={{ center: { x: 126.977, y: 37.568 }, zoom: 14 }}
        >
          <DataLayer
            type="geojson"
            data={geojsonUrl}
            style={style}
            onClick={(event) => {
              const name = event.feature.getProperty("name");
              console.log("선택된 영역:", name);
            }}
          />
        </Map>
      </div>
    </div>
  );
}

export default DataLayerExample;
```

## GeoJSON 라이브 예제 (데이터 직접 전달)

<BrowserOnly>{() => <DataLayerGeoJsonExample />}</BrowserOnly>

## GeoJSON 코드 (데이터 직접 전달)

```tsx
import useBaseUrl from "@docusaurus/useBaseUrl";
import { DataLayer, Map } from "@rousen/react-naver-maps";

function DataLayerExample() {
  const geojson = {
    type: "FeatureCollection",
    features: [
      {
        type: "Feature",
        geometry: {
          type: "Point",
          coordinates: [126.88269675, 37.48146514],
        },
        properties: {
          id: 1408,
          detail: "/course/1408",
        },
      },
      {
        type: "Feature",
        geometry: {
          type: "Point",
          coordinates: [126.91014288, 37.47626925],
        },
        properties: {
          id: 1739,
          detail: "/course/1739",
        },
      },
      {
        type: "Feature",
        geometry: {
          type: "Point",
          coordinates: [126.884041, 37.479118],
        },
        properties: {
          id: 3339,
          detail: "/course/3339",
        },
      },
    ],
  };

  return (
    <div>
      <div style={{ width: "100%", height: "400px" }}>
        <Map
          ncpKeyId="your-ncp-key-id"
          initialOptions={{ center: { x: 126.977, y: 37.568 }, zoom: 14 }}
        >
          <DataLayer
            type="geojson"
            data={geojson}
            style={{
              icon: {
                url: useBaseUrl("img/pin.png"),
              },
            }}
          />
        </Map>
      </div>
    </div>
  );
}

export default DataLayerExample;
```

## GPX 라이브 예제

<BrowserOnly>{() => <DataLayerGpxExample />}</BrowserOnly>

## GPX 코드

```tsx
import { useState } from "react";
import { DataLayer, Map } from "@rousen/react-naver-maps";

function DataLayerGpxExample() {
  const [useAltStyle, setUseAltStyle] = useState(false);
  const style = useAltStyle
    ? {
        strokeColor: "#ef4444",
        strokeOpacity: 0.9,
        strokeWeight: 5,
      }
    : {
        strokeColor: "#22c55e",
        strokeOpacity: 0.9,
        strokeWeight: 3,
      };

  return (
    <div>
      <button type="button" onClick={() => setUseAltStyle((prev) => !prev)}>
        GPX 스타일 변경
      </button>
      <div style={{ width: "100%", height: "400px", marginTop: "10px" }}>
        <Map
          ncpKeyId="your-ncp-key-id"
          initialOptions={{ center: { x: 126.977, y: 37.568 }, zoom: 11 }}
        >
          <DataLayer type="gpx" data="/data/sample-gpx.gpx" style={style} />
        </Map>
      </div>
    </div>
  );
}

export default DataLayerGpxExample;
```

## 설명

- `DataLayer`는 `data`를 fetch해서 `geojson/gpx/kml`을 로딩합니다
- `style`로 피처의 기본 스타일을 지정할 수 있습니다
- `onClick` 등 이벤트 콜백을 등록할 수 있습니다
